...
def signup_screen():
    ...
    # Keep the background behind the signup widgets.
    background_label.lower()
    ...
    # Username entry box
    signup_username = Entry( main_screen, font=("Arial", 12), width=25,
    ...
    # Password entry box
    signup_password = Entry( main_screen, font=("Arial", 12), width=25, show="*",
        bg="#17102B", fg="#FFFFFF", insertbackground="#FFFFFF", relief="flat", bd=0,
        highlightthickness=2, highlightbackground="#9D4EDD", highlightcolor="#7B2CBF")
    signup_password.place(x=400, y=330, anchor="center")

    # Confirm password label
    confirm_label = Label( main_screen, text="CONFIRM PASSWORD", font=("Arial", 10, "bold"), bg="#09051C", fg="#FFFFFF")
    confirm_label.place(x=400, y=365, anchor="center")

    # Confirm password entry box
    signup_confirm = Entry( main_screen, font=("Arial", 12), width=25, show="*",
        bg="#17102B", fg="#FFFFFF", insertbackground="#FFFFFF", relief="flat", bd=0,
        highlightthickness=2, highlightbackground="#9D4EDD", highlightcolor="#7B2CBF")
    signup_confirm.place(x=400, y=395, anchor="center")

    # Signup button
    signup_button = Button( main_screen, text="SIGN UP", command=signup, font=("Arial", 11, "bold"), width=15, height=2,
        bg="#17102B", fg="#FFFFFF", activebackground="#7B2CBF", activeforeground="#FFFFFF", relief="flat", bd=0,
        highlightthickness=2, highlightbackground="#9D4EDD", highlightcolor="#9D4EDD", cursor="hand2")
...
def personal_leaderboard():
    ...
    if len(personal_data) == 0:
        ...
        for i, (player, player_score, time_taken, mode) in enumerate(personal_data, start=1):
            minutes = time_taken // 60
            seconds = time_taken % 60
...
def game_over(reason):
    ...
    Label( main_screen, text=f"Final Score: {game.score}", font=(font, 15)).pack(pady=10 )
...
def show_question_easy(i):
    ...
    lives_label.pack()
    score_label = Label(main_screen, text=f"Score: {game.score}", font=(font,12))
    score_label.pack()
...
# Check if quiz is finished.
    if i >= len(quiz_questions["Easy"]):
        stop_timer()
        save_score()
        Label(main_screen, text=f"Quiz Finished!\nScore: {game.score}/{len(quiz_questions['Easy'])}", font=(font, 18)).pack(pady=20)
        Button( main_screen, text="Play Again", command=start_game_easy).pack(pady=10)
        Button( main_screen, text="Choose Difficulty", command=choose_difficulty_screen ).pack(pady=10)
        Button(main_screen, text="Main Menu", command=main_menu).pack(pady=10)
        return
...
# Show the next question.
    ...
    Button( main_screen, text="Exit Game", command=main_menu).pack(pady=15)
...
# Check if quiz is finished.
    if i >= len(quiz_questions["Medium"]):
        stop_timer()
        save_score()
        Label(main_screen, text=f"Quiz Finished!\nScore: {game.score}/{len(quiz_questions['Medium'])}", font=(font, 18)).pack(pady=20)
        Button( main_screen, text="Play Again", command=start_game_medium).pack(pady=10)
        Button( main_screen, text="Choose Difficulty", command=choose_difficulty_screen ).pack(pady=10)
        Button(main_screen, text="Main Menu", command=main_menu).pack(pady=10)
        return
...
# Check if quiz is finished.
    if i >= len(quiz_questions["Hard"]):
        stop_timer()
        save_score()
        Label(main_screen, text=f"Quiz Finished!\nScore: {game.score}/{len(quiz_questions['Hard'])}", font=(font, 18)).pack(pady=20)
        Button( main_screen, text="Play Again", command=start_game_hard).pack(pady=10)
        Button( main_screen, text="Choose Difficulty", command=choose_difficulty_screen ).pack(pady=10)
        Button(main_screen, text="Main Menu", command=main_menu).pack(pady=10)
        return
