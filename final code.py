...
# This stores information about the game whilst playing.
class Game:
    def __init__(self):
        self.score = 0
        self.lives = 3
...
# Questions for the different difficulties.
quiz_questions = {
    "Easy" : [{
...
def login_screen():
    ...
    image_path = os.path.join( os.path.dirname(os.path.dirname(__file__)) , "Images" , "login_background.png" )
    background_image = Image.open(image_path)
    background_image = background_image.resize((800 , 600))
    background_photo = ImageTk.PhotoImage(background_image)
    background_label = Label(main_screen , image = background_photo)
...
# Checks the saved username and password.
def login():
...
# Stops the timer before clearing the screen. 
...
def show_question_easy(i):
    ...
    if i >= len(quiz_questions["Easy"]):
        ...
        Label( main_screen , text = f"Score: {game.score}/{len(quiz_questions['Easy'])}" , font = (font , 18 , "bold") , bg = "#09051C" , fg = "#C42CFF").place( x = 400 , y = 300 , anchor = "center")
...
def show_question_medium(i):
    ...
    if i >= len(quiz_questions["Medium"]):
        ...
        save_score()
        Label( main_screen , text = "QUIZ FINISHED!" , font = ( font , 24 , "bold" ) , bg = "#09051C" , fg = "White" ).place( x = 400 , y = 250 , anchor = "center" )
        Label( main_screen, text = f"Score: {game.score}/{len(quiz_questions['Medium'])}", font=(font, 18, "bold"), bg = "#09051C" , fg = "#C42CFF").place( x = 400, y = 300, anchor = "center")
